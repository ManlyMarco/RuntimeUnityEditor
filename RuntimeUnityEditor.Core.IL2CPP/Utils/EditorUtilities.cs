...
namespace RuntimeUnityEditor.Core.Utils
{
    ...
    public static class EditorUtilities
    {
        ...
        public static IEnumerable<ReadonlyCacheEntry> GetRootGoScanner()
        {
            ...
            foreach (GameObject go in Resources.FindObjectsOfTypeAll(Il2CppType.Of<GameObject>()).Select(x=>x.Cast<GameObject>()).Where(x => x.transform == x.transform.root))
        ...
        private static IEnumerable<ReadonlyCacheEntry> ScanComponentTypes(IEnumerable<Type> types, bool noTransfroms)
        {
            var allObjects = from type in types
                             let components = Object.FindObjectsOfType(Il2CppType.From(type)).OfType<Component>()
