...
{
    /// <summary>
    /// Shim that deals with some GUILayout methods not being available in IL2CPP.
    /// </summary>
    public static class GUILayoutUtilityShim
    {
        ...
        internal static UnityEngine.Rect GetLastShim(this GUILayoutGroup group)
        {
            if (group.m_Cursor == 0)
            {
            ...
            if (Event.current.type == EventType.Repaint)
                Debug.LogError("Getting control " + group.m_Cursor + "'s position in a group with only " +
                               group.entries.Count + " controls when doing " + Event.current.rawType);
            return new Rect(0, 0, 1, 1);
        }
#endif
    }
